...
async def get_results_average():
    ...
    data = parsing_data("test_database.json")
    ...
    average_value_time_per_output_token = sum(result.time_per_output_token for result in data) / len(data)
    average_value_total_generation_time = sum(result.total_generation_time for result in data) / len(data)
    average_value_prompt_text = sum(len(result.prompt_text) for result in data) / len(data)
    average_value_generated_text = sum(len(result.generated_text) for result in data) / len(data)
...
async def average_over_period_time(start_time: datetime, end_time: datetime):
    ...
    average_value_time_to_first_token = sum(result.time_to_first_token for result in filtered_data) / len(filtered_data)
    average_value_time_per_output_token = sum(result.time_per_output_token for result in filtered_data) / len(
        filtered_data
    )
    average_value_total_generation_time = sum(result.total_generation_time for result in filtered_data) / len(
        filtered_data
    )
    average_value_prompt_text = sum(len(result.prompt_text) for result in filtered_data) / len(filtered_data)
    return {
        "average_value_token_count": average_value_token_count,
        "average_value_time_to_first_token": average_value_time_to_first_token,
        "average_value_time_per_output_token": average_value_time_per_output_token,
