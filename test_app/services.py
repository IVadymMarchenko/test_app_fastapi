...
def calculate_averages_values(data: List[Benchmark]) -> Dict[str, float]:
    ...
    return {
        "average_value_token_count": sum(result.token_count for result in data) / len(data),
...
async def get_filtered_metrics(start_time: datetime, end_time: datetime) -> Dict[str, float]:
    data = await parse_benchmark_data("test_database.json")
