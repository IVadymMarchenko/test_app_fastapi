...
async def load_data_from_json(filename: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to decode JSON in {filename}")


async def parse_benchmark_data(filename: str) -> list[Benchmark]:
    data = await load_data_from_json(filename)
    try:
        return [Benchmark.model_validate(benchmark_data) for benchmark_data in data.get("benchmarking_results", [])]
